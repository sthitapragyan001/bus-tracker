...
export default function MapLeaflet(props) {
  ...
  const updatemap = async () => {
    ...
    //   const startIndex = Math.max(previousIndex - 100, 0);
    //   const endIndex = Math.min(previousIndex + 100, buspath.length - 1);
    ...
    setcoord({ lat: point.latitude, lon: point.longitude })
    // Testing Purpose only 
    // console.log(closestPoint.latitude,closestPoint.longitude)
    //
